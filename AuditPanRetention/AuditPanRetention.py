    ...
    for f in files:
        try:
            with f.open("r", encoding="utf-8", newline="") as fh:
                ...
                bin_col = conf.get("bin_column") or find_column(PAN_BIN_CANDIDATES, header)
                if country_col is None or bin_col is None:
                    continue
                ...
                for row in reader:
                    ...
                    try:
                        ...
                        if country not in allowed:
                            continue
                        bin_val = (row[hmap[bin_col]] or "").strip()
                        flagged.append({
                            "file": f.name,
                            "bin": bin_val,
                            "country": country,
    ...
    with out_file.open("w", encoding="utf-8") as w:
        ...
        if flagged:
            ...
            for it in flagged[:2000]:
                w.write(f"{it['file']}\t{it['bin']}\t{it['country']}\n")
